...
function RetroGrid({
  gridColor = "#c9a227", // Dark academia gold instead of magenta
  showScanlines = true,
  glowEffect = true,
  className = "",
}: RetroGridProps) {
  ...
  useEffect(() => {
    ...
    const hexToRgb = (hex: string) => {
      ...
      return result
        ...
        : { r: 201, g: 162, b: 39 }; // Gold fallback
    };
    ...
    const animate = () => {
      ...
      // Dark academia sky gradient - deep burgundy/purple tones
      const skyGradient = ctx.createLinearGradient(0, 0, 0, canvas.height * 0.55);
      skyGradient.addColorStop(0, `rgba(20, 12, 22, 1)`); // Deep burgundy-black
      skyGradient.addColorStop(0.3, `rgba(30, 18, 33, 1)`);
      skyGradient.addColorStop(0.5, `rgba(42, 26, 46, 1)`); // Burgundy
      skyGradient.addColorStop(0.7, `rgba(${rgb.r * 0.3}, ${rgb.g * 0.25}, ${rgb.b * 0.1}, 1)`);
      skyGradient.addColorStop(0.85, `rgba(${rgb.r * 0.5}, ${rgb.g * 0.4}, ${rgb.b * 0.15}, 1)`);
      skyGradient.addColorStop(1, `rgba(${rgb.r * 0.6}, ${rgb.g * 0.5}, ${rgb.b * 0.2}, 1)`);
      ctx.fillStyle = skyGradient;
      ctx.fillRect(0, 0, canvas.width, canvas.height * 0.55);

      // Ground gradient - dark academia tones
      const groundGradient = ctx.createLinearGradient(0, canvas.height * 0.55, 0, canvas.height);
      groundGradient.addColorStop(0, `rgba(42, 26, 46, 1)`); // Match horizon
      groundGradient.addColorStop(0.3, `rgba(28, 17, 30, 1)`);
      groundGradient.addColorStop(1, `rgba(15, 10, 16, 1)`);
